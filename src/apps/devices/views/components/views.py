from django.shortcuts import render

from apps.devices.models.components.cpu import CPU, CPUModel
from apps.devices.models.components.disk import Disk, DiskModel
from apps.devices.models.components.ram import RAM, RAMModel
from apps.devices.models.components.motherboard import Motherboard, MotherboardModel


def cpu_list(request):
    cpus = CPU.objects.all()
    context = {'cpus': cpus}
    return render(request, 'devices/components/cpu/cpu_list.html', context)


def cpu_model_list(request):
    cpu_models = CPUModel.objects.all()
    context = {'cpu_models': cpu_models}
    return render(request, 'devices/components/cpu/cpu_model.html', context)


def disk_list(request):
    disks = Disk.objects.all()
    context = {'disks': disks}
    return render(request, 'devices/components/disk/disk_list.html', context)


def disk_model_list(request):
    disk_models = DiskModel.objects.all()
    context = {'disk_models': disk_models}
    return render(request, 'devices/components/disk/disk_model.html', context)


def ram_list(request):
    rams = RAM.objects.all()
    context = {'rams': rams}
    return render(request, 'devices/components/ram/ram_list.html', context)


def ram_model_list(request):
    ram_models = RAMModel.objects.all()
    context = {'ram_models': ram_models}
    return render(request, 'devices/components/ram/ram_model.html', context)


def motherboard_list(request):
    motherboards = Motherboard.objects.all()
    context = {'motherboards': motherboards}
    return render(request, 'devices/components/motherboard/motherboard_list.html', context)


def motherboard_model_list(request):
    motherboard_models = MotherboardModel.objects.all()
    context = {'motherboard_models': motherboard_models}
    return render(request, 'devices/components/motherboard/motherboard_model.html', context)
